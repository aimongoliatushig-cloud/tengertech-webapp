...
import { createCalculation, getCalculation, listCalculations, listMaterials, MATERIAL_MODEL, PRICE_MODEL, rpc, updateCalculation, type CalculationPayload } from "@/lib/calculations";
...
export async function DELETE(request: Request) {
  const auth = await authorize(true); if ("error" in auth) return auth.error;
  try { const id = Number(new URL(request.url).searchParams.get("id")); return Response.json({ ok: await rpc<boolean>(auth.session!, "municipal.calculation", "unlink", [[id]]) }); }
  catch (error) { return errorResponse(error); }
}
