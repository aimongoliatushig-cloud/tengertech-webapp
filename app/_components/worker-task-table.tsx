...
export function WorkerTaskTable({
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-[#E7EBE8] bg-white p-6 shadow-[0_1px_2px_rgba(20,40,30,0.04),0_1px_3px_rgba(20,40,30,0.06)]">
      ...
      <div>
        <div className="grid grid-cols-[32px_minmax(0,1fr)_120px_58px] gap-4 border-b border-[#EEF3EF] pb-3 text-[11px] font-semibold uppercase tracking-wide text-[#8A978E] sm:grid-cols-[36px_minmax(0,1fr)_150px_72px]">
          <span>№</span>
          <span>Даалгавар</span>
          <span>Төлөв</span>
          ...
          rows.map((task, index) => {
            ...
            return (
              <Link
                key={`wtt-${task.id}`}
                href={task.href}
                className="group grid min-h-[68px] grid-cols-[32px_minmax(0,1fr)_120px_58px] items-center gap-4 border-b border-[#EEF3EF] py-3.5 transition-colors last:border-b-0 hover:bg-[#F6F8F7] sm:grid-cols-[36px_minmax(0,1fr)_150px_72px]"
              >
