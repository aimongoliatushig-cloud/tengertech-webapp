...
export function WorkerTaskTable({
  ...
  return (
    <section className="min-w-0 rounded-3xl border border-[#E4EFE7] bg-white p-4 shadow-[0_10px_30px_rgba(31,43,36,0.05)] sm:p-5">
          ...
          rows.map((task, index) => {
            ...
            return (
              <Link
                key={`wtt-${task.id}`}
                href={task.href}
                className="group grid grid-cols-[28px_minmax(0,1fr)_112px_24px] items-center gap-3 border-b border-[#EEF3EF] px-1 py-3 transition last:border-b-0 hover:bg-[#F6FBF7] sm:grid-cols-[32px_minmax(0,1fr)_128px_92px_24px]"
              >
                <span
                  className={cn(
